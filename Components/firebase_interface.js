...
//Upload the picture of the user and return the download link
async function UploadSpeakerPhoto(image, name){

    //image name
    let imgName = name + "-" + Math.floor(Math.random() * 100000) + 1; + "-picture";

    //the storage location on firebase
    var storageRef = firebase.storage().ref('/speakers/'+ imgName);
...
async function DropSpeaker(id) {
    await db.collection('speakers').doc(id).delete().then(function() {
        console.log("Document successfully deleted!");
    }).catch(function(error) {
        console.error("Error removing document: ", error);
    });
...
async function UpdateSpeaker(id, data) {
    await db.collection('speakers').doc(id).update(data).then(function() {
        console.log("Document successfully updated!");
    }).catch(function(error) {
        // The document probably doesn't exist.
        console.error("Error updating document: ", error);
